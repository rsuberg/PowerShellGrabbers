    ...
    If ($DomainName -eq $Null) 
        { 
            ...
            If ($UserExist -eq $Null)  
                        { 
                            Write-Host "Error: Username $Username does not exist in $DomainFQDN Domain." -BackgroundColor Red -ForegroundColor White 
                            Rerun 
                            Break 
                        } 
            Else  
                        {    
                            ...
                            Else 
                                    { 
                                        Write-Host "User Enabled: "$UserExist.Enabled -BackgroundColor Red -ForegroundColor White
                                        Write-Host "Enable the user account in Active Directory, Then check again" -BackgroundColor Red -ForegroundColor White
                                        Rerun 
                                        Break 
                                    } 
                            ...
                            If ($UserExist.LockedOut -eq "True") 
                                    { 
                                        ...
                                        Write-Host "Unlock the User Account in Active Directory, Then check again..." -BackgroundColor Red -ForegroundColor White
                                        Rerun 
                                        Break 
                                    } 
                            Else 
            ...
            Write-Host "Authentication failed for $DomainNetBIOS\$UserName with the provided password." -BackgroundColor Red -ForegroundColor White
            Write-Host "Please confirm the password, and try again..." -BackgroundColor Red -ForegroundColor White 
            Rerun 
            Break 
        } 
