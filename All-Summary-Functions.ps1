...
if ($env:OneDriveCommercial -like "*OnlineStorage*") {
 #Run My Unique Commands
...
Function Show-InstalledPrograms {
 Param([switch]$AllFields, [switch]$Table, [switch]$Quiet)
	$List = @()
...
function Show-IP {
 param ([switch]$Virtual, [switch]$Physical, [switch]$Up, [switch]$Down, [switch]$Table, [switch]$List )
...
Function Show-PCSummary {
 param([switch]$SkipMemorySummary)
...
function Show-DriveSummary {
 param ([switch]$List, [switch]$Table, [switch]$Physical)
	hostname
...
Function Show-DriveList {
 # TODO: Explain DriveType, MediaType
	$hdtype = ('Unk', 'SCSI', 'ATAPI', 'ATA', '1394', 'SSA', 'FibChasn', 'USB', 'RAID', 'iSCSI', 'SAS', 'SATA', 'SD', 'MMC', 'Virt-RES', 'FileBacked-Virt', 'StorSpc', 'NVMe')
	$bustype = $hdtype
	$MediaTypeArr = @('Unspecified', "x", "x", 'HDD', 'SSD', 'SCM')
...
Function ExplainArray {
 param ([ValidateSet("HDType", "MediaType", "SysSlotUsage", "ConnType", "BattChemistry", "BattStat", "Availability")]$ArraySel, [int]$ArrayVal) 
